...
static sgx_status_t SGX_CDECL sgx_GeneratePassword(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_GeneratePassword_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_GeneratePassword_t* ms = SGX_CAST(ms_GeneratePassword_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	int* _tmp_random = ms->ms_random;
	size_t _len_random = 64;
	int* _in_random = NULL;
	char* _tmp_Password = ms->ms_Password;
	size_t _len_Password = 64;
	char* _in_Password = NULL;
	uint8_t* _tmp_SealedPwd = ms->ms_SealedPwd;
	uint32_t _tmp_data_size = ms->ms_data_size;
	size_t _len_SealedPwd = _tmp_data_size;
	uint8_t* _in_SealedPwd = NULL;

	CHECK_UNIQUE_POINTER(_tmp_random, _len_random);
	CHECK_UNIQUE_POINTER(_tmp_Password, _len_Password);
	CHECK_UNIQUE_POINTER(_tmp_SealedPwd, _len_SealedPwd);
	...
	if (_tmp_random != NULL && _len_random != 0) {
		if ( _len_random % sizeof(*_tmp_random) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_random = (int*)malloc(_len_random);
		if (_in_random == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_random, _len_random, _tmp_random, _len_random)) {
	...
	if (_tmp_SealedPwd != NULL && _len_SealedPwd != 0) {
		if ( _len_SealedPwd % sizeof(*_tmp_SealedPwd) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_SealedPwd = (uint8_t*)malloc(_len_SealedPwd)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_SealedPwd, 0, _len_SealedPwd);
	}

	GeneratePassword(ms->ms_pwd_size, _in_random, _in_Password, _in_SealedPwd, _tmp_data_size);
	if (_in_Password) {
		if (memcpy_s(_tmp_Password, _len_Password, _in_Password, _len_Password)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
	if (_in_SealedPwd) {
		if (memcpy_s(_tmp_SealedPwd, _len_SealedPwd, _in_SealedPwd, _len_SealedPwd)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
err:
	if (_in_random) free(_in_random);
	if (_in_Password) free(_in_Password);
	if (_in_SealedPwd) free(_in_SealedPwd);
	return status;
}
...
static sgx_status_t SGX_CDECL sgx_get_sealed_data_size(void* pms)
{
	...
	ms->ms_retval = get_sealed_data_size();
...
static sgx_status_t SGX_CDECL sgx_seal_data(void* pms)
{
	...
	ms->ms_retval = seal_data(_in_sealed_blob, _tmp_data_size);
	if (_in_sealed_blob) {
...
static sgx_status_t SGX_CDECL sgx_unseal_data(void* pms)
{
	...
	ms->ms_retval = unseal_data((const uint8_t*)_in_sealed_blob, _tmp_data_size);
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_GeneratePassword, 0, 0},
